...
static void l_CryptoTaOther_Read(CHAR* output, UINT32 len, UINT32 offset)
{
    DMSG("Entry read\n");
    utee_testDriver_read(output, len, offset);
}


static void l_CryptoTaOther_Write(CHAR* input, UINT32 len, UINT32 offset)
{
    DMSG("Entry read\n");
    utee_testDriver_write(input, len, offset);
}


static void l_CryptoTaOther_Dump(CHAR* output, UINT32 len)
{
    DMSG("Entry dump\n");
    utee_testDriver_dump(output, len);
...
int g_CryptoTaHandle_Read(uint32_t paramTypes, TEE_Param params[4])
{
    UINT32 l_Len = 0U;
    UINT32 l_Offset = 0U;
    CHAR* l_pBuf = NULL;
    ...
    /**1) Get the request length & point of responding buffer */
    l_Len = params[0].memref.size;
    l_pBuf = params[0].memref.buffer;
    l_Offset = params[1].value.a;

    l_CryptoTaOther_Read(l_pBuf, l_Len, l_Offset);
    g_TA_printf(l_pBuf, l_Len);

    return OK;
}

int g_CryptoTaHandle_Write(uint32_t paramTypes, TEE_Param params[4])
{
    UINT32 l_Len = 0U;
    UINT32 l_Offset = 0U;
    CHAR* l_pBuf = NULL;
    ...
    l_CryptoTaOther_Write(l_pBuf, l_Len, l_Offset);

    return OK;
}

int g_CryptoTaHandle_Dump(uint32_t paramTypes, TEE_Param params[4])
{
    UINT32 l_Len = 0U;
    CHAR* l_pBuf = NULL;
    uint32_t temp;
    ...
    l_CryptoTaOther_Dump(l_pBuf, l_Len);
    g_TA_printf(l_pBuf, l_Len);
